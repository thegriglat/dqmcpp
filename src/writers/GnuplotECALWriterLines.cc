...
static std::vector<std::array<int, 4>> generateLines(std::vector<std::array<int, 2>> points)
{
...
{
    // contour
    std::vector<std::array<int, 2>> points = {{{50, 0},
            {40, 0},
            {40, 3},
            {35, 3},
            {35, 5},
...
void drawEESM(std::ostream &os, const int iz)
{
    ...
    for (int i = 1; i < 10; ++i) {
        ...
        os << "set label front \"" << ((iz > 0) ? "+" : "-") << std::setw(2) << std::setfill('0') << i << "\" at " << x << "," << y << std::endl;
    }
}
